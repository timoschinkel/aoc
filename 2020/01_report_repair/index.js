...
for(let i = 0; i < input.length; i++) {
    ...
    if (f >= 0) {
        console.log(`Find the two entries that sum to 2020; what do you get if you multiply them together? ${input[i] * input[f]}`);
        break;
    }
}
...
for(let first = 0; first < input.length; first++) {
    ...
    for (let second = first + 1; second < input.length; second++) {
        ...
        if (match != -1) {
            console.log(`What is the product of the three entries that sum to 2020? ${input[first] * input[second] * input[match]}`);
            // artifical break 2
